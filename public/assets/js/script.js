    ...
    fetch("/cart/detail", {
      ...
      .then(data => {
        if(data.code == "success"){
          ...
          cartList.innerHTML = htmlCart.join(""); // Chuyển mảng thành chuổi

          
          // Cập nhật lại Mini Cart
          miniCart.innerHTML = data.cart.length;
          // Hết Cập nhật lại Mini Cart
          ...
          const elementDiscount = document.querySelector("[cart-discount]");
...
//
// Hiển thin tour ra khối giỏ hàng
...
// End Hiển thin tour ra khối giỏ hàng
